...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Armada Kendaraan</h2>
            <p class="text-gray-500 text-sm mt-1">Kelola data truk dan kendaraan operasional KEN Logistics.</p>
        </div>
        <button onclick="openModal('addVehicleModal')" class="flex items-center gap-2 bg-blue-700 text-white px-4 py-2.5 rounded-xl font-semibold shadow-sm hover:bg-blue-800 transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-blue-600">
            ...
            <table class="w-full text-left text-sm text-gray-500">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <span class="font-bold text-gray-900">{{ number_format($vehicle->capacity, 0, ',', '.') }}</span> <span class="text-xs text-gray-500">Kg</span>
                        </td>
                        <td class="px-6 py-4">
                            @php
                                $statusColors = [
                                ...
                                $colorClass = $statusColors[$vehicle->status] ?? 'bg-gray-100 text-gray-700 border-gray-200';
                            @endphp
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border {{ $colorClass }}">
                                {{ $vehicle->status }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <button onclick="editVehicle({{ json_encode($vehicle) }})" class="p-2 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-colors" title="Edit">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-400">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm">Klik tombol "Tambah Armada" untuk memasukkan data truk.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
