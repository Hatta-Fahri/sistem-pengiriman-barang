                    ...
                    <div class="overflow-x-auto max-h-[600px] overflow-y-auto">
                        <table class="w-full text-left text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100 text-gray-700">
                                ...
                                <tr class="hover:bg-blue-50/50 cursor-pointer" onclick="document.getElementById('ship_{{ $shipment->id }}').click()">
                                    ...
                                    <td class="px-6 py-4"><div class="font-semibold">{{ $shipment->receiver_name }}</div><div class="text-xs text-gray-500">{{ $shipment->destination_city }}</div></td>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden sticky top-6">
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            <input type="text" name="jalur_pengiriman" value="{{ old('jalur_pengiriman', $manifest->jalur_pengiriman) }}" required class="w-full rounded-xl border-gray-300 focus:ring-blue-500 text-sm shadow-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="vehicle_id" id="vehicle_select" required class="w-full rounded-xl border-gray-300 focus:ring-blue-500 text-sm shadow-sm">
                                @foreach($availableVehicles as $vehicle)
                                    <option value="{{ $vehicle->id }}" data-capacity="{{ $vehicle->capacity }}" {{ $manifest->vehicle_id == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->license_plate }} (Max: {{ number_format($vehicle->capacity, 0) }}Kg)</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="courier_id" required class="w-full rounded-xl border-gray-300 focus:ring-blue-500 text-sm shadow-sm">
                                @foreach($availableCouriers as $courier)
                                    <option value="{{ $courier->id }}" {{ $manifest->courier_id == $courier->id ? 'selected' : '' }}>{{ $courier->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="pt-4 border-t border-gray-100">
                            <button type="submit" id="btn_submit" class="w-full py-3.5 bg-blue-700 text-white rounded-xl font-black text-sm uppercase tracking-wide hover:bg-blue-800 transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">Update Jadwal</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function calculateCapacity() {
            ...
            const maxCapacity = selectedOption ? parseFloat(selectedOption.getAttribute('data-capacity') || 0) : 0;
