...
@section('content')
    <div class="w-full space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <a href="{{ route('manifests.create') }}"
                class="flex items-center gap-2 bg-blue-700 text-white px-4 py-2.5 rounded-xl font-semibold shadow-sm hover:bg-blue-800 transition-colors">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-500">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($manifests as $manifest)
                            <tr class="hover:bg-blue-50/30 transition-colors group">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-[11px] text-gray-500 mt-0.5">Dibuat:
                                        {{ $manifest->created_at->format('d M Y') }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span
                                            class="text-xs font-medium text-orange-600 bg-orange-50 px-2.5 py-1 rounded-md border border-orange-100">Belum
                                            Dialokasikan</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="flex justify-between items-end mb-1">
                                        <span class="text-xs font-bold text-gray-700">
                                            {{ number_format($totalWeight, 1) }} <span class="text-gray-400 font-normal">/
                                                {{ number_format($capacity, 0) }} Kg</span>
                                        </span>
                                    ...
                                    <div class="text-[10px] text-gray-400 mt-1 text-right">
                                        {{ $manifest->total_shipments ?? 0 }} Resi Dimuat</div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-black uppercase {{ $statusColor }}">
                                        {{ $manifest->status }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end items-center gap-2">
                                        ...
                                        @if ($manifest->status === 'Persiapan')
                                            <form action="{{ route('manifests.destroy', $manifest->id) }}" method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-400 italic font-medium">Belum ada
                                    jadwal keberangkatan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
