...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Manajemen Kurir</h2>
            <p class="text-gray-500 text-sm mt-1">Kelola data personil kurir, lisensi berkendara, dan status kerja.</p>
        </div>
        <button onclick="openModal('addCourierModal')" class="flex items-center gap-2 bg-blue-700 text-white px-4 py-2.5 rounded-xl font-semibold shadow-sm hover:bg-blue-800 transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-blue-600">
            ...
            <table class="w-full text-left text-sm text-gray-500">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="font-bold text-gray-900 uppercase">{{ $courier->courier_code ?? 'KRR-NEW' }}</div>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <button onclick="editCourier({{ json_encode($courier) }})" class="p-2 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-colors" title="Edit Akun">
                                    <i data-lucide="edit-3" class="w-4 h-4"></i>
                                </button>

                                <form action="{{ route('couriers.destroy', $courier->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menonaktifkan akun kurir ini?');">
                                    @csrf @method('DELETE')
